...
class ShellAsyncTerminal:
    ...
    def historyup(self):
        ...
        if self.historyind == -1:
            ...
            self.historyind -= 1
        if self.historyind in range(len(self.history)):
    ...
    def sendhistoryup(self):
        ...
        if self.sendhistoryind == -1:
            ...
            self.sendhistoryind -= 1
        if self.sendhistoryind in range(len(self.sendhistory)):
    ...
    def execute(self):
        ...
        if command.startswith("cd"):
            ...
            if len(command.split(" ")) == 2 and not ('&' in command or ';' in command):
                ...
                p.wait()
                for l in p.stderr.read().decode('utf-8', 'ignore').split("\n"):
                    if len(l) > 0:
                        self.commandAppend(l)
                pwd = p.stdout.read().decode('utf-8', 'ignore').split("\n")
                if len(pwd) > 1:
                    self.cwd = pwd[-2]
